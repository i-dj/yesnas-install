...
main() {
  ...
  [[ "$EUID" -eq 0 ]] || { command -v sudo >/dev/null 2>&1 || fail "sudo is required."; sudo -v; }
  ...
  log "YesNAS version ${version} will be installed."
  ...
  YESNAS_USER="$RUN_USER" YESNAS_HOSTNAME="$DEVICE_NAME" setsid bash "$server_installer" </dev/null
  ...
  YESNAS_WEB_USER="$RUN_USER" setsid bash "$web_installer" </dev/null
